...
async function updateLeadClickCount(leadEmail: string) {
  try {
    ...
    const searchResponse = await fetch(
      `https://api.airtable.com/v0/${AIRTABLE_BASE_ID}/Leads?filterByFormula={Email}='${leadEmail}'`,
      {
        headers: {
          'Authorization': `Bearer ${AIRTABLE_API_KEY}`,
        }
    ...
    if (!searchResponse.ok) {
      console.error('Error searching for lead:', searchResponse.status);
      return false;
    }
    ...
    if (searchData.records && searchData.records.length > 0) {
      ...
      const updateResponse = await fetch(
        `https://api.airtable.com/v0/${AIRTABLE_BASE_ID}/Leads/${record.id}`,
        {
          method: 'PATCH',
          headers: {
            'Authorization': `Bearer ${AIRTABLE_API_KEY}`,
